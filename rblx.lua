...
║    ╚═════╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚══════╝                        ║
║                                                                      ║
║             UNIVERSAL  •  WORKS ALL ROBLOX GAMES                    ║
║                      SSS PREMIUM TIER                                ║
║                                                                      ║
╠══════════════════════════════════════════════════════════════════════╣
║  MOVE TAB:                                                           ║
║   ‣ Speed (AssemblyLinearVelocity, WalkSpeed stays 16)               ║
║   ‣ Fly (BodyVelocity + BodyGyro, cam-relative)                      ║
║   ‣ Ghost Noclip (real stepped loop)                                 ║
║   ‣ Godmode (clone-desync method)                                    ║
║   ‣ Infinite Jump, No Fall Damage, Low Gravity                       ║
║   ‣ FreeCam (detached camera, TP back)                               ║
║   ‣ Hitbox Expander (scale hitboxes for easy hits)                   ║
║   ‣ Anti-Void (prevent falling out of map)                           ║
║                                                                      ║
║  VISUAL TAB:                                                         ║
║   ‣ ESP Player (SelectionBox + BillboardGui + distance + health)     ║
║   ‣ Chams (player silhouette through walls)                          ║
║   ‣ ESP Objects (SelectionBox + labels)                              ║
║   ‣ Aimbot (raycast wall-check + exponential smoothing + FOV)        ║
║   ‣ Full Bright, No Fog, Custom Sky                                  ║
║   ‣ Tracers (line from screen center to targets)                     ║
║                                                                      ║
║  PLAYER TAB:                                                         ║
║   ‣ Scanner Objek Universal (filter, sort, TP, ESP toggle)           ║
║   ‣ Player List (TP to player, bring, copy pos, spectate)            ║
║   ‣ Rejoin / Server Hop                                              ║
║   ‣ Anti-AFK (auto move prevention kick)                             ║
║   ‣ Walk Animation Changer                                           ║
║                                                                      ║
║  EXPLOIT TAB:                                                        ║
║   ‣ Auto Discover Remotes (keyword highlight, auto-add to claim)     ║
║   ‣ Auto Claim Loop (configurable delay)                             ║
║   ‣ Instant Interact / ProximityPrompt                               ║
║   ‣ Remote Tester (fire, invoke, spam x10)                           ║
║   ‣ Kill Aura (damage via tool or touch)                             ║
║   ‣ Anti-Kick hook                                                   ║
║                                                                      ║
║  ARCHITECTURE:                                                       ║
║   ‣ Single Heartbeat (ActiveFeatures table)                          ║
║   ‣ Maid class (zero memory leak)                                    ║
║   ‣ Config: writefile/readfile JSON persistent                       ║
║   ‣ Identity: instance names randomised each run                     ║
║   ‣ UI: flat layout — ZERO nested AutomaticSize bug                  ║
║   ‣ MkTog returns (button, setter, getter) — no instance hacking     ║
╚══════════════════════════════════════════════════════════════════════╝
]]
...
local ContentPrv    = game:GetService("ContentProvider")

-- ═══════════════════════════════════════════════════════════════════════
--  §1  LOCALIZE  (micro-optimization)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §2  IDENTITY  (randomise names to avoid game scanners)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §3  MAID  (zero-leak connection / instance manager)
-- ═══════════════════════════════════════════════════════════════════════
local Maid = {}
Maid.__index = Maid
...
-- ═══════════════════════════════════════════════════════════════════════
--  §4  CONFIG  (persistent JSON via exploit writefile/readfile)
-- ═══════════════════════════════════════════════════════════════════════
local Def = {
    speedVal  = 120,
    flySpd    = 120,
...
-- ═══════════════════════════════════════════════════════════════════════
--  §6  ACTIVE FEATURES TABLE  (single Heartbeat loop for all features)
-- ═══════════════════════════════════════════════════════════════════════
local AF = {}

-- ═══════════════════════════════════════════════════════════════════════
--  §7  CLEANUP OLD GUI INSTANCES
-- ═══════════════════════════════════════════════════════════════════════
...
local C = {
    BG    = C3rgb(8,   8,   13),
    BG2   = C3rgb(13,  13,  20),
    Panel = C3rgb(17,  17,  27),
    Card  = C3rgb(22,  22,  34),
    Card2 = C3rgb(28,  28,  42),
    Bord  = C3rgb(40,  40,  62),
    Bord2 = C3rgb(55,  55,  80),

    Acc   = C3rgb(0,   200, 120),
    AccG  = C3rgb(0,   255, 160),
    AccD  = C3rgb(0,   130, 75),

    Red   = C3rgb(205, 45,  45),
    RedL  = C3rgb(240, 60,  60),
    Grn   = C3rgb(30,  170, 70),
    Blu   = C3rgb(45,  110, 240),
    BluL  = C3rgb(70,  140, 255),
    Pur   = C3rgb(135, 55,  225),
    PurL  = C3rgb(165, 90,  255),
    Ora   = C3rgb(240, 148, 0),
    OraL  = C3rgb(255, 175, 30),
    Yel   = C3rgb(230, 195, 30),
    Cya   = C3rgb(0,   205, 225),
    CyaL  = C3rgb(30,  235, 255),
    Pnk   = C3rgb(215, 75,  160),
    PnkL  = C3rgb(245, 100, 190),

    Txt   = C3rgb(220, 220, 230),
    TxtD  = C3rgb(115, 115, 140),
    TxtM  = C3rgb(60,  60,  82),
    Inp   = C3rgb(11,  11,  18),
    Inp2  = C3rgb(16,  16,  26),
}

-- Tab accent colors (4 tabs)
...
-- ═══════════════════════════════════════════════════════════════════════
--  §14  TOGGLE FACTORY
--  ▸ BUG FIX: Returns (button, setter, getter) — never stores functions
--             as Instance properties (invalid in Roblox).
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §16  MAIN WINDOW  (W×H draggable frame)
-- ═══════════════════════════════════════════════════════════════════════
local W, H = 346, 545
...
-- fix rounded corners clash with glow bar
...
-- bottom accent line + sliding indicator
...
-- ═══════════════════════════════════════════════════════════════════════
--  §18  PAGE AREA  (4 ScrollingFrames)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §19  SECTION BUILDER
--  ▸ header (TextButton) + body (Frame, AutomaticSize.Y) + spacer
--    All three are DIRECT children of the ScrollingFrame — no nesting.
-- ═══════════════════════════════════════════════════════════════════════
local secOrder = {}
...
local function MkSec(page, title, color, pageIdx)
    ...
    -- Body (AutomaticSize.Y — direct child, NOT nested)
...
-- ── MOVEMENT ──────────────────────────────────────────────────────────
-- Speed row (input + toggle side by side)
...
fcSpdW.LayoutOrder = 1; fcSpdW.Size = UDim2n(0,87,0,34)

local fcSnsInp, fcSnsW = MkInp(fcInputRow, "Sens", tostring(Cfg.fcSns), UDim2n(0,87,0,34))
fcSnsW.LayoutOrder = 2; fcSnsW.Size = UDim2n(0,87,0,34)
...
local aimInfoLbl = Lbl(bAim,
    "T = Lock/Unlock  •  G = Cycle target",
...
local sTpBtn = MkBtn(sBRow, "TELEPORT", C.Card, UDim2n(0,88,1,0), 11); sTpBtn.LayoutOrder  = 2; sTpBtn.TextColor3 = C.TxtD
local sEBtn  = MkBtn(sBRow, "ESP: OFF", C.Card, UDim2n(0,68,1,0), 11); sEBtn.LayoutOrder   = 3; sEBtn.TextColor3  = C.TxtD
...
local plrTpBtn     = MkBtn(plrActRow, "TP TO",    C.Pur,          UDim2n(0,74,1,0),10); plrTpBtn.LayoutOrder     = 1; plrTpBtn.TextColor3   = C.TxtD
local plrBringBtn  = MkBtn(plrActRow, "BRING",    C.Red:Lerp(C.Pur,.3), UDim2n(0,66,1,0),10); plrBringBtn.LayoutOrder  = 2; plrBringBtn.TextColor3 = C.TxtD
local plrSpecBtn   = MkBtn(plrActRow, "SPECTATE", C.Blu,          UDim2n(0,78,1,0),10); plrSpecBtn.LayoutOrder   = 3; plrSpecBtn.TextColor3  = C.TxtD

-- ═══════════════════════════════════════════════════════════════════════
--  §24  PAGE 4 — EXPLOIT  (sections)
-- ═══════════════════════════════════════════════════════════════════════
...
rDelW.LayoutOrder = 1; rDelW.Size = UDim2n(0,90,0,34)
local rClrBtn = MkBtn(rDelRow, "CLEAR LIST", C.Red:Lerp(C.Card,.4), UDim2n(0,90,0,34), 10)
rClrBtn.LayoutOrder = 2; rClrBtn.TextColor3 = C.TxtD
...
Cor(ieRadW2,7); Str(ieRadW2,C.Bord,1)
...
local rtPathInp, _ = MkInp(bRT, "Remote path...", "", nil, 1); _.LayoutOrder = 1
local rtArgInp,  _ = MkInp(bRT, 'Args JSON: null or [1,"abc"]', "null", nil, 2); _.LayoutOrder = 2
...
local rtSpam = MkBtn(rtActRow, "SPAM ×10",    C.Red:Lerp(C.Ora,.5), UDim2n(0,80,1,0), 11); rtSpam.LayoutOrder  = 2
...
-- ═══════════════════════════════════════════════════════════════════════
--  §26  FEATURE: SPEED  (AssemblyLinearVelocity — WalkSpeed stays 16)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §27  FEATURE: FLY  (BodyVelocity + BodyGyro, camera-relative)
-- ═══════════════════════════════════════════════════════════════════════
local function ClrFly()
    local c = lp.Character; if not c then return end
    local r = c:FindFirstChild("HumanoidRootPart")
    if r then
        local bv = r:FindFirstChild("_CFV"); if bv then bv:Destroy() end
        local bg = r:FindFirstChild("_CFG"); if bg then bg:Destroy() end
    end
...
-- ═══════════════════════════════════════════════════════════════════════
--  §28  FEATURE: GHOST NOCLIP  (real stepped loop per-part)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §29  FEATURE: GODMODE  (clone-desync)
-- ═══════════════════════════════════════════════════════════════════════
...
AF.antivoid = function()
    ...
    if hrp.Position.Y < -200 then
        ...
        SetSt("Anti-Void: Rescued from void!", C.Yel)
    end
end
...
afkTog.MouseButton1Click:Connect(function()
    ...
    if St.afkOn then
        ...
        St.afkConn = M:Add(RunService.Heartbeat:Connect(function()
            local t = os.clock()
            -- Every 55 seconds do a tiny virtual camera rotation
            if t % 55 < 0.05 then
                pcall(function()
                    local vr = Instance.new("VirtualInputManager")
                    -- fallback: move humanoid direction slightly
                    local h = GM()
                    if h then
...
-- ═══════════════════════════════════════════════════════════════════════
--  §38  FEATURE: AIMBOT  (raycast wall-check + exponential smooth)
-- ═══════════════════════════════════════════════════════════════════════
...
M:Add(UIS.InputBegan:Connect(function(i, gp)
    ...
    if i.KeyCode == Enum.KeyCode.T then
        if St.lockTgt then
            St.lockTgt = nil; St.aimOn = false; setAim(false)
            lockLbl.Text = "Target: —"
        else
            local t = FindNrst()
            if t then
                St.lockTgt = t; St.aimOn = true; setAim(true)
                lockLbl.Text = "Target: " .. t.name
            end
        end
    end
...
AF.aimbot = function()
    ...
    if not HasLOS(camPos, tPos) then
        lockLbl.TextColor3 = C.Red; return
    end
...
-- ═══════════════════════════════════════════════════════════════════════
--  §39  FEATURE: ESP PLAYERS  (SelectionBox + BillboardGui + health)
-- ═══════════════════════════════════════════════════════════════════════
...
-- Chams (force highlight through walls)
...
-- ═══════════════════════════════════════════════════════════════════════
--  §40  FEATURE: TRACERS  (line from screen bottom-center to targets)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
--  §43  FEATURE: SCANNER (object browser)
-- ═══════════════════════════════════════════════════════════════════════
local scanEspAct = {}
local scanHLBox  = nil
...
local function BuildSList(flt)
    ...
    for idx, data in ipairs(sorted) do
        ...
        e.MouseButton1Click:Connect(function()
            ...
            TW(sTpBtn,  .15, { BackgroundColor3 = C.Grn }); sTpBtn.TextColor3 = C.Txt
        end)
    end
...
sTpBtn.MouseButton1Click:Connect(function()
    ...
    local pos = GPos(St.scanSel); if not pos then sStatL.Text = "No position!"; return end
...
sEBtn.MouseButton1Click:Connect(function()
    ...
    if scanEspAct[obj] then
        ...
        TW(sEBtn, .15, { BackgroundColor3 = C.Card }); sEBtn.TextColor3 = C.TxtD; sEBtn.Text = "ESP: OFF"
    else
        AddEspO(obj, C.Yel); scanEspAct[obj] = true
        TW(sEBtn, .15, { BackgroundColor3 = C.Blu }); sEBtn.TextColor3 = C.Txt; sEBtn.Text = "ESP: ON"
    end
end)

-- ═══════════════════════════════════════════════════════════════════════
--  §44  FEATURE: PLAYER LIST
-- ═══════════════════════════════════════════════════════════════════════
local function BuildPlrList()
    ...
    for i, p in ipairs(ps) do
        ...
        selBtn.MouseButton1Click:Connect(function()
            ...
            TW(plrTpBtn,    .15, { BackgroundColor3 = C.Pur }); plrTpBtn.TextColor3    = C.Txt
            TW(plrBringBtn, .15, { BackgroundColor3 = C.Red }); plrBringBtn.TextColor3 = C.Txt
            TW(plrSpecBtn,  .15, { BackgroundColor3 = C.Blu }); plrSpecBtn.TextColor3  = C.Txt
        end)
    end
...
plrTpBtn.MouseButton1Click:Connect(function()
    local p = St.plrSel; if not p then plrStatL.Text = "Pilih player dulu!"; return end
    local c = p.Character; if not c then return end
    local r = c:FindFirstChild("HumanoidRootPart"); if not r then return end
    local hrp = GH(); if hrp then hrp.CFrame = r.CFrame * CF(0,0,-3) end
    SetSt("TP → " .. p.Name)
end)
...
-- ═══════════════════════════════════════════════════════════════════════
--  §45  FEATURE: AUTO DISCOVER REMOTES
-- ═══════════════════════════════════════════════════════════════════════
local KW = {
...
local function ScanRem()
    ...
    pcall(function()
        local gs = game:GetService("Players")
        for _, ch in pairs(gs:GetChildren()) do
            if ch:IsA("RemoteFunction") or ch:IsA("RemoteEvent") then
                table.insert(out, { name=ch.Name, path="PL."..ch.Name, inst=ch, type=ch.ClassName })
            end
        end
    end)
    return out
...
local function AddREntry(d)
    ...
    local nameLbl = Lbl(e, d.name, UDim2n(0.48,0,1,0), hl and C.Ora or C.Txt, Enum.Font.GothamSemibold, 10)
    nameLbl.Position      = UDim2n(0,36,0,0)
...
rAutoTog.MouseButton1Click:Connect(function()
    ...
    if St.acOn then
        if #St.claimList == 0 then
            ...
            setRATog(false); St.acOn = false; acRun = false; return
        end
        task.spawn(function()
...
-- ═══════════════════════════════════════════════════════════════════════
--  §46  FEATURE: INSTANT INTERACT  (ProximityPrompt)
-- ═══════════════════════════════════════════════════════════════════════
...
ieFireBtn.MouseButton1Click:Connect(function()
    ...
    for _, pp in pairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            local pt = pp.Parent
            if pt and pt:IsA("BasePart") and
               (pt.Position - hrp.Position).Magnitude <= radius then
                pp.HoldDuration = 0
...
rtFire.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local ok, res = pcall(function()
            ...
            elseif rem:IsA("RemoteEvent") then rem:FireServer(table.unpack(args)); return "fired"
            end
        end)
...
rtSpam.MouseButton1Click:Connect(function()
    local pth = rtPathInp.Text; if pth == "" then return end
    local rem = ResolvePath(pth)
    if not rem then rtResL.Text = "Tidak ditemukan!"; return end
    local args    = ParseArgs(rtArgInp.Text)
    local success = 0
    rtResL.Text       = "Spamming ×10..."
...
akTog.MouseButton1Click:Connect(function()
            ...
            pcall(function()
                ...
                mt.__namecall = newcclosure(function(self, ...)
                    ...
                    if method == "Kick" and self == lp then
                        akStatL.Text = "Kick blocked! (" .. tostring(select(1,...)):sub(1,40) .. ")"
                        return
                    end
    ...
    elseif not St.akOn then
        ...
        SetSt("Anti-Kick OFF (hook stays active)")
    end
end)

-- ═══════════════════════════════════════════════════════════════════════
--  §50  SINGLE HEARTBEAT LOOP  (drives all AF.* features)
-- ═══════════════════════════════════════════════════════════════════════
M:Add(RunService.Heartbeat:Connect(function(dt)
    -- Non-camera features run in Heartbeat
    for _, fn in pairs({
        AF.speed,
        AF.nofall,
        AF.godmode,
...
-- Camera-affecting features run in RenderStepped for smooth visuals
...
BtnClose.MouseButton1Click:Connect(function()
    -- Restore game state before unloading
    if St.godOn  then GodOff() end
...
M:Add(lp.CharacterAdded:Connect(function(char)
    ...
    if St.godOn then
        St.godOn = false; St.gReal = nil; St.gFake = nil
        setGod(false)
    end
    if St.flyOn then
        St.flyOn = false; setFly(false)
        ClrFly()
    end
    if St.fcOn then
        StopFC(); setFC(false)
    ...
    for p, _ in pairs(St.espAvas) do RmEspA(p) end
    for p, h in pairs(chamHighlights) do
        pcall(function() h:Destroy() end); chamHighlights[p] = nil
    end
    SetSt("Respawn detected — state auto-adjusted")
end))

-- ═══════════════════════════════════════════════════════════════════════
--  ANIMATED GLOW BAR PULSE (Fix Penutup)
-- ═══════════════════════════════════════════════════════════════════════
local glowT = 0
M:Add(RunService.RenderStepped:Connect(function(dt)
    glowT = glowT + dt
    ...
    if gBar then
        gBar.BackgroundTransparency = 0.22 - pulse
    end
end))

SetSt("CYRUZ SSS Premium Loaded", C.Acc)
